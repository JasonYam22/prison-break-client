...
function Navbar() {
  ...
  return (
    <nav className="relative flex justify-between items-center p-4 border-b border-amber-50 prison-font text-3xl">
      ...
      <Link
        to="/"
        className="prison-font text-4xl tracking-widest text-red-800"
      >
        PRISON BREAK
      </Link>
      ...
      <button
        ...
        className="text-2xl hover:scale-125 transition-all duration-300 rounded-2xl"
      >
        ☰
      </button>
    ...
    ${menuOpen ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-90 -translate-y-8 pointer-events-none"}`}
      >
        <Link
          to="/about"
          className="px-2 py-1 hover:bg-gray-700 text-xl rounded-2xl"
        >
          About
        </Link>
      </div>
